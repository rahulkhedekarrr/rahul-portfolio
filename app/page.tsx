...
const Navigation = memo(({ items }: { items: NavItem[] }) => {
  return (
    <motion.nav
      className="relative z-10 p-6 smooth-scroll"
    ...
    >
      <div className="max-w-7xl mx-auto">
        <motion.div
          className="group backdrop-blur-optimized rounded-2xl border border-white/20 shadow-xl p-4 hover-optimized gpu-accelerated"
...
const ContactLink = memo(({ link }: { link: ContactLink }) => {
  ...
  return (
    <motion.a
      href={link.href}
      className="group flex items-center space-x-3 backdrop-blur-optimized border border-white/20 rounded-2xl p-4 hover-optimized gpu-accelerated"
      whileHover={{
        scale: 1.05,
        transition: { duration: 0.2, ease: "easeOut" },
      }}
